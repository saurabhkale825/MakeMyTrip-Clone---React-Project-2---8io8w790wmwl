...
function SourceTrainStationList() {

    const {source , setSource} = useContext(TrainContext);
    ...
    <div className='train-station'>
        <ul>
            {cities.map((item , index) => (
                <li className='individual-station' onClick={() => setSource(item.JunctionName)} key={index}>{item.JunctionName} </li>
            ))}
        </ul>
    </div>
  )
...
export default SourceTrainStationList